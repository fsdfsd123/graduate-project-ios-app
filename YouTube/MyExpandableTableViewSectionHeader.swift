...
final class MyExpandableTableViewSectionHeader: LUExpandableTableViewSectionHeader {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
//        let notificationName = Notification.Name("Expand")
//        NotificationCenter.default.addObserver(self, selector: #selector(Expand(noti:)), name:notificationName, object: nil)
        label?.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(didTapOnLabel)))
        label?.isUserInteractionEnabled = true
    }
    
    // MARK: - IBActions
//    func Expand(notification: NSNotification){
//        let thesection = noti.userInfo!["Section"] as! Int
//       
//        delegate?.expandableSectionHeader(self, shouldExpandOrCollapseAtSection: thesection)
//    }
