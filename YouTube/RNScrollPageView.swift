...
class RNScrollPageView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    @IBInspectable open var selectedIndex: Int = platformpage // selected index, default is 0
    ...
    @IBInspectable open var subLineViewColor: UIColor = UIColor.white // subLineView's color, default is red
    @IBInspectable open var titleColorNormal: UIColor = UIColor.white // item's titleColor for normal, default is black
    @IBInspectable open var titleColorSelected: UIColor = UIColor.orange // item's titleColor for selected, default is red
    ...
    @IBInspectable open var splitLineViewColor: UIColor = UIColor.gray // the bottom splitLineView's background color, default is white
    ...
    // MARK: - private method
     ...
     func configScrollPage() {
        ...
        self.collectionView?.backgroundColor = UIColor.rbg(r: 109, g: 109, b: 109)
    ...
    private func getPageItemWidths() -> [CGFloat] {
        let widths = self.titles.map { (title) -> CGFloat in
            ...
            let rect = title.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: self.pageItemHeight), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: font], context: nil)
            return rect.width + self.itemTitleMargin
        }
        return widths
    }
    
     func showSelectedViewController(_ selectedIndex: Int) {
    ...
    private func updatePageItemState(_ index: Int, scrollView: UIScrollView) {
     ...
     func updatePageItemState(_ formIndex: Int, toIndex: Int) {
        //update titleColor
...
extension UIColor {
    ...
    func transFormColorTo(_ color: UIColor, progress: CGFloat) -> UIColor? {
        ...
        let newColor = UIColor.init(_colorLiteralRed: Float(newR), green: Float(newG), blue: Float(newB), alpha: Float(alpha))
