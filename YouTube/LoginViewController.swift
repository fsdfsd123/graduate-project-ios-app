    ...
    @objc func handleSignIn() {
        ...
        Auth.auth().signIn(withEmail: email, password: pass) { user, error in
            if error == nil && user != nil {
                ...
                ilivenetUser.uid = user?.uid ?? "error"
                database.child("users").child("\(user?.uid)").observeSingleEvent(of: .value, with: { (snapshot) in
                    // Get user value
                    let value = snapshot.value as? NSDictionary
                 
                    
                    ilivenetUser.name = value?["displayName"] as? String ?? ""
                    ilivenetUser.email = value?["email"] as? String ?? ""
                    ilivenetUser.photoURL = value?["photoURL"] as? String ?? ""
                    // ...
                }) { (error) in
                    print(error.localizedDescription)
                }
                ilivenetUser.history = [history(video: FsdVideo(), videourl: "", starttime: Date(), endtime: Date(), duration: 0)]//初始化history
